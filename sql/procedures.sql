...
JOIN (
	SELECT
		...
		COUNT(distinct(v_upgrades_matches.match_id)) - 1 AS game_count ,
		...
		COUNT(distinct CASE WHEN v_upgrades_matches.match_type = 2 THEN CONCAT(v_upgrades_matches.match_id,v_upgrades_matches.player_id) ELSE 0 END) - 1 AS cut_games,
		COUNT(distinct CASE WHEN v_upgrades_matches.match_type = 2 AND v_upgrades_matches.player_id = v_upgrades_matches.winner_id THEN CONCAT(v_upgrades_matches.match_id,v_upgrades_matches.player_id) ELSE 0 END) -1 AS cut_wins
...
FROM (
		...
		AND tournaments.players > 10
		AND tournaments.fill_rate > .65
...
COMMENT ''
SELECT 
	ref_pilot.ref_pilot_id AS pilot1_xws,
	scenario AS scenario,
	COUNT(distinct(v_pilots_matches.match_id)) AS games,
	COUNT(distinct CASE WHEN v_pilots_matches.player_id = v_pilots_matches.winner_id THEN v_pilots_matches.match_id ELSE 0 END) - 1 AS wins,
	(COUNT(distinct CASE WHEN v_pilots_matches.player_id = v_pilots_matches.winner_id THEN v_pilots_matches.match_id ELSE 0 END) - 1) / COUNT(distinct(v_pilots_matches.match_id)) AS winrate
FROM v_pilots_matches
JOIN ref_pilot ON ref_pilot.ref_pilot_id = v_pilots_matches.ref_pilot_id
AND ref_pilot.xws = xws
AND v_pilots_matches.date >= start_date
AND v_pilots_matches.date <= end_date
AND v_pilots_matches.format = format
GROUP BY ref_pilot.ref_pilot_id, scenario
...
SELECT 
	...
	1 - upgrade_stats.swiss_result / upgrade_stats.swiss_count AS avg_percentile,
	...
	0.0 + upgrade_stats.list_cut_count / upgrade_stats.lists_with_cut AS cut_rate,
...
WHERE ref_upgrade_type.upgrade_type_id = (SELECT ref_upgrade_type.upgrade_type_id FROM ref_upgrade_type WHERE ref_upgrade_type.name = input_type)
AND list_count > 0
ORDER BY list_count desc;

DROP PROCEDURE IF EXISTS atc.GetPilotStatsByUpgrade;
